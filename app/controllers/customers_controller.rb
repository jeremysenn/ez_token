...
#      @payment_transactions =  Kaminari.paginate_array(@account.payment_transactions.sort_by(&:date_time).reverse).page(params[:payments]).per(10)
      @payment_and_fee_transactions =  Kaminari.paginate_array(@account.payment_and_fee_transactions.sort_by(&:date_time).reverse).page(params[:payments]).per(10)
...
#      format.html { redirect_to customers_url, notice: 'Customer was successfully destroyed.' }
      format.html { redirect_to accounts_url, notice: 'Customer was successfully destroyed.' }
