...
#    Rails.logger.debug "ezcash_quick_pay_web_service_call response body: #{response.body}"
  ...
  def self.to_csv
    require 'csv'
    attributes = %w{tranID date_time Description dev_id error_code tran_code sec_tran_code from_acct_id to_acct_id source amt_req amt_auth ChpFee receipt_nbr Note}
