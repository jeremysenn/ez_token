...
#  end
  ...
  def one_time_payment(amount, note, receipt_number, event_id, device_id, from_customer_id, to_customer_id)
    client = Savon.client(wsdl: "#{ENV['EZCASH_WSDL_URL']}")
    response = client.call(:ez_cash_txn, message: { FromActID: company.transaction_account.blank? ? nil : company.transaction_account.id, ToActID: account.id, Amount: amount, Fee: 0, FeeActId: company.fee_account.blank? ? nil : company.fee_account.id, Note: note, ReceiptNbr: receipt_number, event_id: event_id, dev_id: device_id, FromCustID: from_customer_id, ToCustID: to_customer_id})
  ...
  def one_time_payment_with_no_text_message(amount, note, receipt_number, event_id, device_id, from_customer_id, to_customer_id)
    client = Savon.client(wsdl: "#{ENV['EZCASH_WSDL_URL']}")
